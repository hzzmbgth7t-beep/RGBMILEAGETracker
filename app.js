const APP_NAME="RGB Mileage", VERSION="2.1.6s", SCHEMA_VERSION=VERSION, BUILD_DATE="2026-06-12", KEY="RGBM_DATA_v213d";
...
initV213aShell();initV213Shell();applyDisplayModeClass();window.addEventListener("resize",()=>setTimeout(applyDisplayModeClass,25),{passive:true});window.addEventListener("orientationchange",()=>setTimeout(applyDisplayModeClass,25),{passive:true});initV213eStabilization();state=loadData();render();if('serviceWorker' in navigator){navigator.serviceWorker.register('sw.js?v=216s').catch(()=>{})}