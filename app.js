const VERSION="2.1.3a", BUILD_DATE="2026-06-07", KEY="RGBM_DATA_v213a";
const LEGACY_KEYS=["RGBM_DATA_v213","RGBM_DATA_v212d","RGBM_DATA_v212c","RGBM_DATA_v212b","RGBM_DATA_v212a","RGBM_DATA_v212","RGBM_DATA_v211","RGBM_DATA_v210","rgbMileage","rgbm_data_v110","rgbMileage_v2_0_6","rgbMileage_v2_0_7","rgbMileage_v2_0_8","rgbMileage_v2_0_9","rgbMileage_v2_0_10","rgbMileage_v2_0_11"];
...
function blankData(){return {fuelGrades:[...FUEL_GRADES],app:"RGB Mileage",schemaVersion:"2.1.3a",appVersion:VERSION,settings:{lastBackupDate:"",showArchived:false},nextEntrySequence:1,stations:[...STATIONS_DEFAULT],maintenanceCategories:[...MAINT_CATS],vehicles:[null,null],vehicleAcquisitionRecords:[],fuelRecords:[],maintenanceRecords:[],insuranceRecords:[],attachments:[],createdAt:nowISO(),modifiedAt:nowISO()}}

function normalizeData(input){
  const d=blankData();
  if(!input || typeof input!=="object") return d;
  d.app="RGB Mileage";
  d.schemaVersion="2.1.3a";
...
function saveRecordEdit(type,recordId){
  ...
  nav("vehicleView",{vehicleId:r.vehicleId});
}
...
function activeList(listName){
  if(listName==="fuelGrades"){if(!state.fuelGrades)state.fuelGrades=[...FUEL_GRADES];return state.fuelGrades}
  if(listName==="stations"){if(!state.stations)state.stations=[...STATIONS_DEFAULT];return state.stations}
  if(listName==="maintenanceCategories"){if(!state.maintenanceCategories)state.maintenanceCategories=[...MAINT_CATS];return state.maintenanceCategories}
...
function otherSaveToList(){
  ...
  const val=otherEnteredValue();
  if(!val)return alert("Enter a value first.");
  const list=activeList(pendingOtherSelect.listName);
...
function settings(app){app.innerHTML=header("Settings")+`<div class="card"><h2>Vehicle Order</h2><button class="wide" onclick="swapVehicles()">Swap Vehicle Circles</button></div><div class="card"><h2>About</h2><p>RGB Mileage v${VERSION}<br>Build Date: ${BUILD_DATE}<br>Schema: 2.1.3a</p><button class="wide danger" onclick="if(confirm('Clear all local data?')){localStorage.removeItem(KEY);state=blankData();saveData();nav('home')}">Clear Local Data</button></div>`+bottomNav()+footer()} function swapVehicles(){[state.vehicles[0],state.vehicles[1]]=[state.vehicles[1],state.vehicles[0]];state.vehicles.forEach((v,i)=>{if(v)v.slot=i});saveData();alert("Vehicle order swapped.");nav("home")}
initV213aShell();initV213Shell();state=loadData();render();if('serviceWorker' in navigator){navigator.serviceWorker.register('sw.js?v=211').catch(()=>{})}