const APP_NAME="RGB Mileage", VERSION="2.1.5", SCHEMA_VERSION=VERSION, BUILD_DATE="2026-06-09", KEY="RGBM_DATA_v213d";
...
function goHome(){if(route.screen==="quickFuel")return handleQuickFuelHome();if(route.screen==="quickMaintenance")return handleQuickMaintenanceHome();return nav("home")}
function goBack(){if(route.screen==="recordEdit")return handleRecordEditBack();if(route.screen==="quickFuel")return handleQuickFuelBack();if(route.screen==="quickMaintenance")return handleQuickMaintenanceBack();performBackNavigation()}
...
function openRecordFlow(type,recordId,mode){
  if(type==="Fuel"){
...
function vehicleTap(i){if(suppressTap){suppressTap=false;return} const v=state.vehicles[i]; if(v)openFuelFromHome(v.vehicleId); else nav("vehicleEdit",{slot:i})} function vehicleLong(i){const v=state.vehicles[i]; if(v)nav("vehicleView",{vehicleId:v.vehicleId}); else nav("vehicleEdit",{slot:i})}
...
function recordView(app,type,id){const r=findRecord(type,id);if(!r)return nav("home",{},false);app.innerHTML=header("View "+type+" Record")+`<div class="card">${meta(r)}<h3>${type} Information</h3><div class="readonly-grid">${viewFields(type,r)}</div><div class="view-actions"><button class="danger" onclick="archiveRecord('${type}','${id}')">Archive</button><button onclick="navRecord('${type}','${id}','edit')">Edit</button></div></div>`+bottomNav()+footer()} function viewFields(type,r){if(type==="Fuel")return roBox("Date",r.date)+roBox("Time",r.time)+roBox("Odometer",fmt(r.odometer))+roBox("Miles",fmt(r.miles))+roBox("Gallons",fmt(r.gallons,3))+roBox("MPG",fmt(r.mpg))+roBox("Fuel Grade",r.fuelGrade)+roBox("Ethanol Free",r.ethanolFree)+roBox("Station",r.station)+roBox("Cost Source",r.fuelCostSource)+roBox("Price/Gal",money(r.fuelPricePerGallon))+roBox("Total Cost",money(r.totalFuelCost))+roBox("Notes",r.notes); if(type==="Maintenance")return roBox("Date",r.dropOffDate||r.date)+roBox("Category",r.category)+roBox("Odometer",fmt(r.odometer))+roBox("Cost",money(r.totalCost||r.cost))+roBox("Location",r.location)+roBox("Provider",r.serviceProvider||r.provider)+roBox("Pickup Date",r.pickUpDate)+roBox("Performed By",r.performedBy)+roBox("Notes",r.notes); if(type==="Insurance")return roBox("Agency",r.agency||r.company)+roBox("Policy Number",r.policyNumber)+roBox("Effective Date",r.effectiveDate)+roBox("Expiration Date",r.expirationDate)+roBox("Agreed Value",money(r.agreedValue!==""&&r.agreedValue!=null?r.agreedValue:(r.coverageValue!==""&&r.coverageValue!=null?r.coverageValue:r.insuranceValue)))+roBox("Premium",money(r.premium))+roBox("Agent",r.agent||r.agentName)+roBox("Phone",r.phone)+roBox("Email",r.email)+roBox("Notes",r.notes||r.coverageNotes);return ""}
...
function fuelFormHtml(vid,r,readOnly){
  ...
  return `<div class="form-grid">
    <label>Date<input type="date" id="fdate" value="${esc(r.date||"")}"${readOnly?' readonly':''}></label>
    <label>Time<input type="time" id="ftime" value="${esc(r.time||"")}"${readOnly?' readonly':''}></label>
    <label>Odometer<input type="number" step="0.01" id="fodo" value="${esc(r.odometer||"")}" ${ro}${readOnly?'':' oninput="calcFuel(\''+vid+'\')"'}></label>
    <label>Miles<input type="number" step="0.01" id="fmiles" value="${esc(r.miles||"")}" ${ro}${readOnly?'':' oninput="calcMpg()"'}></label>
    <label>Gallons<input type="number" step="0.001" id="fgal" value="${esc(r.gallons||"")}" ${ro}${readOnly?'':' oninput="calcMpg()"'}></label>
    <label>MPG<input type="number" step="0.01" id="fmpg" value="${esc(r.mpg||"")}" readonly></label>
    <label>Fuel Grade<select id="fgrade"${dis} ${readOnly?'':'onfocus="this.setAttribute(\'data-prev\',this.value)" onchange="selectOther(this,\'fuelGrades\')"'}>${gradeList.map(g=>`<option ${g===(r.fuelGrade||"")?'selected':''}>${esc(g)}</option>`).join("")}</select></label>
    <label>Ethanol Free<select id="fef"${dis}><option ${String(r.ethanolFree||"")===""?"selected":""}></option><option ${String(r.ethanolFree||"")==="Yes"?"selected":""}>Yes</option><option ${String(r.ethanolFree||"")==="No"?"selected":""}>No</option></select></label>
    <label>Station<select id="fstation"${dis} ${readOnly?'':'onfocus="this.setAttribute(\'data-prev\',this.value)" onchange="selectOther(this,\'stations\')"'}>${stationList.map(s=>`<option ${s===(r.station||"")?'selected':''}>${esc(s)}</option>`).join("")}</select></label>
    <label>Cost Source<select id="fcostsource"${dis}><option ${String(r.fuelCostSource||"")===""?"selected":""}></option><option ${String(r.fuelCostSource||"")==="Calculated"?"selected":""}>Calculated</option><option ${String(r.fuelCostSource||"")==="Entered"?"selected":""}>Entered</option></select></label>
    <label>Price/Gal<input type="number" step="0.01" id="fprice" value="${esc(r.fuelPricePerGallon||"")}" ${ro}${readOnly?'':' oninput="calcCost()"'}></label>
    <label>Total Cost<input type="number" step="0.01" id="fcost" value="${esc(r.totalFuelCost||"")}" ${ro}></label>
    <label class="full">Notes<textarea id="fnotes"${readOnly?' readonly':''}>${esc(r.notes||"")}</textarea></label>
  </div>`;
}
...
function maintenanceFormHtml(vid,r,readOnly){
  ...
  const dis=readOnly?' disabled':''; const ro=readOnly?' readonly':'';
  return `<div class="form-grid">
    <label>Date<input type="date" id="mdrop" value="${esc(r.dropOffDate||r.date||"")}"${readOnly?' readonly':''}></label>
    <label>Pickup Date<input type="date" id="mpick" value="${esc(r.pickUpDate||"")}"${readOnly?' readonly':''}></label>
    <label>Category<select id="mcat"${dis} ${readOnly?'':'onfocus="this.setAttribute(\'data-prev\',this.value)" onchange="selectOther(this,\'maintenanceCategories\')"'}>${catList.map(c=>`<option ${c===(r.category||"")?'selected':''}>${esc(c)}</option>`).join("")}</select></label>
    <label>Odometer<input type="number" step="0.01" id="modo" value="${esc(r.odometer||"")}" ${ro}></label>
    <label>Cost<input type="number" step="0.01" id="mcost" value="${esc(r.totalCost||r.cost||"")}" ${ro}></label>
    <label>Location<input id="mloc" value="${esc(r.location||"")}" ${ro}></label>
    <label>Provider<input id="mprov" value="${esc(r.serviceProvider||r.provider||"")}" ${ro}></label>
    <label>Performed By<input id="mperf" value="${esc(r.performedBy||"")}" ${ro}></label>
    <label class="full">Notes<textarea id="mnotes"${readOnly?' readonly':''}>${esc(r.notes||"")}</textarea></label>
  </div>`;
}
...
function quickInsurance(app,vid){app.innerHTML=header("Insurance Entry")+`<div class="card"><div class="form-grid"><label>Agency<input id="iagency"></label><label>Policy Number<input id="ipol"></label><label>Effective Date<input type="date" id="ieff"></label><label>Expiration Date<input type="date" id="iexp"></label><label>Agreed Value<input type="number" step="0.01" id="iagree"></label><label>Premium<input type="number" step="0.01" id="iprem"></label><label>Agent<input id="iagent"></label><label>Phone<input id="iphone"></label><label>Email<input type="email" id="iemail"></label><label class="full">Notes<textarea id="inotes"></textarea></label></div><button class="wide primary" onclick="saveQuickInsurance('${vid}')">Save Policy</button><button class="wide ghost" onclick="nav('home')">Cancel</button></div>`+previousRecordsHtml("Insurance",vid)+bottomNav()+footer()} function saveQuickInsurance(vid){try{const agency=requireValue($("iagency").value,"Agency");const policyNumber=requireValue($("ipol").value,"Policy Number");const effectiveDate=requireValue($("ieff").value,"Effective Date");const expirationDate=cleanText($("iexp").value);if(expirationDate&&expirationDate<effectiveDate)throw new Error("Expiration Date cannot be earlier than Effective Date.");const agreedValue=requireNonNegative($("iagree").value,"Agreed Value");const premium=requireNonNegative($("iprem").value,"Premium");const agent=cleanText($("iagent").value);const notes=cleanText($("inotes").value);const r=baseRecord("Insurance",vid,"Manual Entry");Object.assign(r,{company:agency,agency,policyNumber,effectiveDate,expirationDate,agreedValue,coverageValue:agreedValue,insuranceValue:agreedValue,premium,agent,agentName:agent,phone:cleanText($("iphone").value),email:cleanText($("iemail").value),notes,coverageNotes:notes,attachments:[]});state.insuranceRecords.push(r);saveData();alert("Insurance policy saved.");nav("vehicleView",{vehicleId:vid})}catch(e){alert(e.message||String(e))}}
