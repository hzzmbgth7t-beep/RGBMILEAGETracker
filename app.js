const APP_NAME="RGB Mileage", VERSION="2.1.6l", DISPLAY_VERSION="2.1.6l-wc06", SCHEMA_VERSION=VERSION, BUILD_DATE="2026-06-12", KEY="RGBM_DATA_v213d";
...
function home(app){
  const slots=state.vehicles.map((v,i)=>`<div class="vehicle-slot vehicle-slot-${i+1}">${circleHtml(v,i)}</div>`).join("");
  app.innerHTML=`<div class="screen home"><div class="home-header"><h1 class="chrome-title">${APP_NAME}</h1><div class="subtitle version-subtitle">v${DISPLAY_VERSION} • Build ${formatBuildDate(BUILD_DATE)}</div></div><div class="home-content"><div class="vehicle-layout">${slots}</div></div></div>${bottomNav()}`
}
