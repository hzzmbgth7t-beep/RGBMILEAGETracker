const APP_NAME="RGB Mileage", VERSION="2.1.3od", SCHEMA_VERSION=VERSION, BUILD_DATE="2026-06-09", KEY="RGBM_DATA_v213d";
...
function goBack(){if(route.screen==="recordEdit")return handleRecordEditBack();performBackNavigation()}
...
function rowPressStart(type,recordId,e){
  pressStart(()=>navRecord(type,recordId,"edit"),e,650);
}
...
function meta(r){return `<details class="data-info"><summary>Data Information</summary><div class="readonly-grid"><div class="fieldbox"><b>Record ID</b><span>${esc(r.recordId)}</span></div><div class="fieldbox"><b>Sequence</b><span>${r.entrySequence}</span></div><div class="fieldbox"><b>Source</b><span>${esc(r.source)}</span></div><div class="fieldbox"><b>Quality</b><span>${esc(r.dataQuality)}</span></div><div class="fieldbox"><b>Tags</b><span>${esc(tags(r.classificationTags).join('; '))}</span></div></div></details>`} function roBox(label,val){return `<div class="fieldbox"><b>${esc(label)}</b><span>${esc(val??"")}</span></div>`}
...
initV213aShell();initV213Shell();initV213eStabilization();state=loadData();render();if('serviceWorker' in navigator){navigator.serviceWorker.register('sw.js?v=213od').catch(()=>{})}