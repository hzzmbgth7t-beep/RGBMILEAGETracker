const VERSION="2.1.3", BUILD_DATE="2026-06-07", KEY="RGBM_DATA_v213";
const LEGACY_KEYS=["RGBM_DATA_v212d","RGBM_DATA_v212c","RGBM_DATA_v212b","RGBM_DATA_v212a","RGBM_DATA_v212","RGBM_DATA_v211","RGBM_DATA_v210","rgbMileage","rgbm_data_v110","rgbMileage_v2_0_6","rgbMileage_v2_0_7","rgbMileage_v2_0_8","rgbMileage_v2_0_9","rgbMileage_v2_0_10","rgbMileage_v2_0_11"];
...
function blankData(){return {app:"RGB Mileage",schemaVersion:"2.1.3",appVersion:VERSION,settings:{lastBackupDate:"",showArchived:false},nextEntrySequence:1,stations:[...STATIONS_DEFAULT],maintenanceCategories:[...MAINT_CATS],vehicles:[null,null],vehicleAcquisitionRecords:[],fuelRecords:[],maintenanceRecords:[],insuranceRecords:[],attachments:[],createdAt:nowISO(),modifiedAt:nowISO()}}

function normalizeData(input){
  const d=blankData();
  if(!input || typeof input!=="object") return d;
  d.app="RGB Mileage";
  d.schemaVersion="2.1.3";
  ...
  try{
    ...
    if(/quota|exceeded|storage/i.test(msg)){
      ...
      try{
        ...
        throw new Error("Storage quota exceeded. Backup size "+Math.round(payload.length/1024)+" KB. Old RGB Mileage storage was cleared, but Safari still refused the restore.");
      }
    }else{
      throw e;
    }
...
function normMaint(r,vid){const rec=normRecord(r,"Maintenance",vid);Object.assign(rec,{date:r.date||r.dropOffDate||"",dropOffDate:r.dropOffDate||r.date||r.drop||"",pickUpDate:r.pickUpDate||r.pick||"",category:r.category||r.service||"Maintenance",status:r.status||"",odometer:numVal(r.odometer),location:r.location||"",serviceProvider:r.serviceProvider||r.provider||"",provider:r.provider||r.serviceProvider||"",performedBy:r.performedBy||"",totalCost:numVal(r.totalCost||r.cost),cost:numVal(r.totalCost||r.cost),attachments:arr(r.attachments)});return rec}
...
function pressStart(cb,e){if(e&&e.preventDefault)e.preventDefault();clearLP();suppressTap=false;longTimer=setTimeout(()=>{suppressTap=true;cb()},500)}
function rowTap(e,cb){if(suppressTap){if(e&&e.preventDefault)e.preventDefault();suppressTap=false;return}if(cb)cb()}
...
function vehicleView(app,vid){const v=getVehicle(vid); if(!v)return nav("home",{},false); const acq=getAcq(vid); app.innerHTML=header("View Vehicle")+`<div class="card"><h2>${esc(vehicleLabel(v))}</h2><div class="readonly-grid"><div class="fieldbox"><b>Year</b><span>${esc(v.year)}</span></div><div class="fieldbox"><b>Make</b><span>${esc(v.make)}</span></div><div class="fieldbox"><b>Model</b><span>${esc(v.model)}</span></div><div class="fieldbox"><b>Status</b><span>${esc(v.status)}</span></div><div class="fieldbox"><b>Acquisition Date</b><span>${esc(acq.acquisitionDate||"")}</span></div><div class="fieldbox"><b>Purchase Price</b><span>${esc(acq.purchasePrice||"")}</span></div><div class="fieldbox full"><b>Seller</b><span>${esc(acq.seller||"")}</span></div></div><div class="form-actions"><button onclick="nav('vehicleEdit',{vehicleId:'${vid}'})">Edit</button><button class="danger" onclick="archiveVehicle('${vid}')">Archive</button></div></div><div class="card"><h2>Actions</h2><button class="wide" onclick="nav('quickFuel',{vehicleId:'${vid}'})">Quick Fuel Entry</button><button class="wide" onclick="nav('quickMaintenance',{vehicleId:'${vid}'})">Quick Maintenance Entry</button><button class="wide" onclick="nav('quickInsurance',{vehicleId:'${vid}'})">Quick Insurance Entry</button></div>`+previousRecordsHtml("Fuel",vid)+previousRecordsHtml("Maintenance",vid)+previousRecordsHtml("Insurance",vid)+bottomNav()+footer()}
...
function entryRow(type,r){const b=tags(r.classificationTags).map(t=>`<span class="badge ${t==='Historical'?'warn':t==='Archived'?'arch':''}">${esc(t)}</span>`).join("");return `<div class="entry-row" onpointerdown="pressStart(()=>nav('recordEdit',{type:'${type}',recordId:'${r.recordId}'}),event)" onpointerup="clearLP()" onpointercancel="clearLP()" onclick="rowTap(event,()=>nav('recordView',{type:'${type}',recordId:'${r.recordId}'}))"><div class="entry-main"><span>${recordTitle(type,r)}</span><span class="muted">${esc(r.dataQuality)}</span></div><div class="badges">${b}</div></div>`}
...
function commonEdit(r){return `<label>Data Quality<select id="rq">${DATA_QUALITIES.map(q=>`<option ${r.dataQuality===q?'selected':''}>${q}</option>`).join("")}</select></label><label>Tags<input id="rtags" value="${esc(tags(r.classificationTags).join('; '))}"></label><label>Notes<textarea id="rnotes">${esc(r.notes||"")}</textarea></label>`} function recordEdit(app,type,id){const r=findRecord(type,id);if(!r)return nav("home",{},false);app.innerHTML=header("Edit "+type+" Record")+`<div class="card">${editFields(type,r)}${commonEdit(r)}<div class="form-actions"><button class="primary" onclick="saveRecordEdit('${type}','${id}')">Save Changes</button><button class="danger" onclick="archiveRecord('${type}','${id}')">Archive</button></div></div>`+bottomNav()+footer()}
...
function otherUseOnce(){
  ...
  addOptionToSelect(pendingOtherSelect.sel,val);
  otherClose(); pendingOtherSelect=null;
}
function otherSaveToList(){
  if(!pendingOtherSelect)return;
  const val=otherEnteredValue();
  if(!val)return alert("Enter a value first.");
  const listName=pendingOtherSelect.listName;
  if(!state[listName])state[listName]=[];
  if(!state[listName].some(x=>String(x).toLowerCase()===val.toLowerCase()))state[listName].push(val);
  addOptionToSelect(pendingOtherSelect.sel,val);
  saveData();
  otherClose(); pendingOtherSelect=null;
}
...
function previousFuelOdo(vid,odo){const rows=records("Fuel",vid,false).filter(r=>r.odometer!==""&&Number(r.odometer)<Number(odo)).sort((a,b)=>Number(b.odometer)-Number(a.odometer));return rows[0]?.odometer??""} function calcFuel(vid){const odo=numVal($("fodo").value),prev=previousFuelOdo(vid,odo);if(odo!==""&&prev!=="")$("fmiles").value=(odo-prev).toFixed(2);calcMpg()} function calcMpg(){const m=numVal($("fmiles").value),g=numVal($("fgal").value);$("fmpg").value=(m!==""&&g>0)?(m/g).toFixed(2):"";calcCost()} function calcCost(){const g=numVal($("fgal")?.value),p=numVal($("fprice")?.value);if($("fcost")&&g!==""&&p!=="")$("fcost").value=(g*p).toFixed(2)} function saveQuickFuel(vid){const r=baseRecord("Fuel",vid,"Manual Entry");Object.assign(r,{date:$("fdate").value,time:$("ftime").value,odometer:numVal($("fodo").value),miles:numVal($("fmiles").value),gallons:numVal($("fgal").value),mpg:numVal($("fmpg").value),fuelGrade:$("fgrade").value,ethanolFree:$("fef").value,station:$("fstation").value,fuelPricePerGallon:numVal($("fprice").value),totalFuelCost:numVal($("fcost").value),fuelCostSource:$("fcost").value?"Calculated":"",notes:$("fnotes").value,attachments:[]});if(r.odometer!==""&&r.miles==="")addTag(r,"Historical");state.fuelRecords.push(r);saveData();alert("Fuel entry saved.");nav("home")}
function quickMaintenance(app,vid){app.innerHTML=header("Quick Maintenance Entry")+`<div class="card"><label>Date<input type="date" id="mdrop"></label><label>Category<select id="mcat" onchange="openOtherList('mcat','maintenanceCategories','Maintenance Category')">${state.maintenanceCategories.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join("")}</select></label><label>Odometer<input type="number" step="0.01" id="modo"></label><label>Cost<input type="number" step="0.01" id="mcost"></label><label>Notes<textarea id="mnotes"></textarea></label><button class="wide primary" onclick="saveQuickMaintenance('${vid}')">Save Maintenance</button><button class="wide ghost" onclick="nav('home')">Clear & Exit</button></div>`+previousRecordsHtml("Maintenance",vid)+bottomNav()+footer()} function saveQuickMaintenance(vid){const r=baseRecord("Maintenance",vid,"Manual Entry");Object.assign(r,{date:$("mdrop").value,dropOffDate:$("mdrop").value,category:$("mcat").value||"Maintenance",odometer:numVal($("modo").value),totalCost:numVal($("mcost").value),cost:numVal($("mcost").value),location:$("mloc").value,serviceProvider:$("mprov").value,provider:$("mprov").value,notes:$("mnotes").value,attachments:[]});state.maintenanceRecords.push(r);saveData();alert("Maintenance saved.");nav("vehicleView",{vehicleId:vid})}
function quickInsurance(app,vid){app.innerHTML=header("Quick Insurance Entry")+`<div class="card"><label>Company<input id="icomp"></label><label>Policy Number<input id="ipol"></label><div class="row"><label>Effective Date<input type="date" id="ieff"></label><label>Expiration Date<input type="date" id="iexp"></label></div><label>Premium<input type="number" step="0.01" id="iprem"></label><label>Notes<textarea id="inotes"></textarea></label><button class="wide primary" onclick="saveQuickInsurance('${vid}')">Save Policy</button><button class="wide ghost" onclick="nav('home')">Clear & Exit</button></div>`+previousRecordsHtml("Insurance",vid)+bottomNav()+footer()} function saveQuickInsurance(vid){const r=baseRecord("Insurance",vid,"Manual Entry");Object.assign(r,{company:$("icomp").value,policyNumber:$("ipol").value,effectiveDate:$("ieff").value,expirationDate:$("iexp").value,coverageValue:numVal($("icov").value),insuranceValue:numVal($("icov").value),premium:numVal($("iprem").value),agent:$("iagent").value,agentName:$("iagent").value,phone:$("iphone").value,email:$("iemail").value,agency:$("iagency").value,notes:$("inotes").value,attachments:[]});state.insuranceRecords.push(r);saveData();alert("Insurance policy saved.");nav("vehicleView",{vehicleId:vid})}
...
function settings(app){app.innerHTML=header("Settings")+`<div class="card"><h2>Vehicle Order</h2><button class="wide" onclick="swapVehicles()">Swap Vehicle Circles</button></div><div class="card"><h2>About</h2><p>RGB Mileage v${VERSION}<br>Build Date: ${BUILD_DATE}<br>Schema: 2.1.3</p><button class="wide danger" onclick="if(confirm('Clear all local data?')){localStorage.removeItem(KEY);state=blankData();saveData();nav('home')}">Clear Local Data</button></div>`+bottomNav()+footer()} function swapVehicles(){[state.vehicles[0],state.vehicles[1]]=[state.vehicles[1],state.vehicles[0]];state.vehicles.forEach((v,i)=>{if(v)v.slot=i});saveData();alert("Vehicle order swapped.");nav("home")}
initV213Shell();state=loadData();render();if('serviceWorker' in navigator){navigator.serviceWorker.register('sw.js?v=211').catch(()=>{})}