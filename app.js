const APP_NAME="RGB Mileage", VERSION="2.1.7b", SCHEMA_VERSION=VERSION, BUILD_DATE="2026-06-12", KEY="RGBM_DATA_v213d";
...
function home(app){app.innerHTML=`<div class="screen home home-shell"><div class="home-content-shell"><div class="home-head"><h1 class="chrome-title">${APP_NAME}</h1><div class="subtitle version-subtitle">v${VERSION} • Build ${formatBuildDate(BUILD_DATE)}</div></div><div class="vehicle-area">${state.vehicles.map((v,i)=>circleHtml(v,i)).join("")}</div></div><div class="home-menu-rail">${bottomNav()}</div></div>`}
...
function isStandaloneDisplayMode(){
  try{
    ...
    const refStandalone = String(document.referrer||"").startsWith("android-app://");
    return !!(mm || iosStandalone || refStandalone);
...
function applyDisplayModeClass(){
  try{
    ...
    document.documentElement.classList.toggle("browser-mode",!standalone);
  }catch(e){}
}
...
initV213aShell();initV213Shell();applyDisplayModeClass();window.addEventListener("resize",()=>setTimeout(applyDisplayModeClass,25),{passive:true});window.addEventListener("orientationchange",()=>setTimeout(applyDisplayModeClass,25),{passive:true});window.addEventListener("pageshow",()=>setTimeout(applyDisplayModeClass,25),{passive:true});document.addEventListener("visibilitychange",()=>setTimeout(applyDisplayModeClass,25),{passive:true});initV213eStabilization();state=loadData();render();if('serviceWorker' in navigator){navigator.serviceWorker.register('sw.js?v=217b').catch(()=>{})}