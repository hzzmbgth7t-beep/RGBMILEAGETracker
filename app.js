const VERSION="2.1.3e", BUILD_DATE="2026-06-08", KEY="RGBM_DATA_v213d";
...
function blankData(){return {fuelGrades:[...FUEL_GRADES],app:"RGB Mileage",schemaVersion:"2.1.3e",appVersion:VERSION,settings:{lastBackupDate:"",showArchived:false},nextEntrySequence:1,stations:[...STATIONS_DEFAULT],maintenanceCategories:[...MAINT_CATS],vehicles:[null,null],vehicleAcquisitionRecords:[],fuelRecords:[],maintenanceRecords:[],insuranceRecords:[],attachments:[],createdAt:nowISO(),modifiedAt:nowISO()}}

function normalizeData(input){
  const d=blankData();
  if(!input || typeof input!=="object") return d;
  d.app="RGB Mileage";
  d.schemaVersion="2.1.3e";
...
function rowTap(e,cb){if(suppressTap){suppressTap=false;if(e&&e.preventDefault)e.preventDefault();return}if(cb)cb()}
function entryRow(type,r){const b=tags(r.classificationTags).map(t=>`<span class="badge ${t==='Historical'?'warn':t==='Archived'?'arch':''}">${esc(t)}</span>`).join("");return `<div class="entry-row" onclick="rowTap(event,()=>navRecord('${type}','${r.recordId}','view'))" onpointerdown="pressStart(()=>navRecord('${type}','${r.recordId}','edit'),event)" onpointerup="clearLP()" onpointercancel="clearLP()" onpointerleave="clearLP()" oncontextmenu="event.preventDefault()"><div class="entry-main"><span>${recordTitle(type,r)}</span><span class="muted">${esc(r.dataQuality||"")}</span></div><div class="badges">${b}</div></div>`}
...
function settings(app){app.innerHTML=header("Settings")+`<div class="card"><h2>Vehicle Order</h2><button class="wide" onclick="swapVehicles()">Swap Vehicle Circles</button></div><div class="card"><h2>About</h2><p>RGB Mileage v${VERSION}<br>Build Date: ${BUILD_DATE}<br>Schema: 2.1.3e</p><button class="wide danger" onclick="if(confirm('Clear all local data?')){localStorage.removeItem(KEY);state=blankData();saveData();nav('home')}">Clear Local Data</button></div>`+bottomNav()+footer()} function swapVehicles(){[state.vehicles[0],state.vehicles[1]]=[state.vehicles[1],state.vehicles[0]];state.vehicles.forEach((v,i)=>{if(v)v.slot=i});saveData();alert("Vehicle order swapped.");nav("home")}
...
initV213aShell();initV213Shell();initV213eStabilization();state=loadData();render();if('serviceWorker' in navigator){navigator.serviceWorker.register('sw.js?v=213e').catch(()=>{})}