const APP_NAME="RGB Mileage", VERSION="2.1.3i", BUILD_DATE="2026-06-09", KEY="RGBM_DATA_v213d";
...
function blankData(){return {fuelGrades:[...FUEL_GRADES],app:"RGB Mileage",schemaVersion:"2.1.3i",appVersion:VERSION,settings:{lastBackupDate:"",showArchived:false},nextEntrySequence:1,stations:[...STATIONS_DEFAULT],maintenanceCategories:[...MAINT_CATS],vehicles:[null,null],vehicleAcquisitionRecords:[],fuelRecords:[],maintenanceRecords:[],insuranceRecords:[],attachments:[],createdAt:nowISO(),modifiedAt:nowISO()}}

function normalizeData(input){
  const d=blankData();
  if(!input || typeof input!=="object") return d;
  d.app="RGB Mileage";
  d.schemaVersion="2.1.3i";
...
function rowPressStart(type,recordId,e){
  ...
  rowInteraction.timer=setTimeout(()=>{
    ...
    nav('recordEdit',{type,recordId}, false);
  },500);
}
...
function vehiclePressStart(i,e){
  if(e && e.cancelable) e.preventDefault();
  if(rowInteraction.timer) clearTimeout(rowInteraction.timer);
  rowInteraction={timer:null,long:false,type:'Vehicle',recordId:String(i)};
  rowInteraction.timer=setTimeout(()=>{
    rowInteraction.long=true;
    vehicleLong(i);
  },500);
}
function vehiclePressEnd(i,e){
  if(rowInteraction.timer) clearTimeout(rowInteraction.timer);
  const wasLong=rowInteraction.long;
  rowInteraction={timer:null,long:false,type:null,recordId:null};
  if(wasLong) return false;
  nav('quickFuel',{vehicleId:state.vehicles[i].vehicleId});
  return false;
}
function vehiclePressCancel(){
  if(rowInteraction.timer) clearTimeout(rowInteraction.timer);
  rowInteraction={timer:null,long:false,type:null,recordId:null};
}
...
function openOtherModal(sel,listName){
  ...
  div.innerHTML=`<div class="other-modal"><h2>${esc(title)}</h2><label>${esc(label)}<input id="otherModalInput" autocomplete="off"></label><div class="other-actions"><button class="primary" onclick="otherUseOnce()">Use Once</button><button onclick="otherSaveToList()">Save To List</button><button class="ghost" onclick="cancelRecordEdit()" onclick="cancelRecordEdit()">Cancel</button></div></div>`;
