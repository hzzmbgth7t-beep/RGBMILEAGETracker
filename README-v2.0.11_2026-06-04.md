...
RGB Mileage v2.0.11 replaces prompt-based fuel-entry editing with a unified Quick Fuel Entry/Edit workflow. Editing a previous fuel entry now opens the same Quick Fuel Entry form, populates it with the selected entry, changes the save button to Save Edit, and keeps the user on the Quick Fuel Entry page after saving.

## Current Features

- Vehicle-circle Home Screen remains the primary navigation.
- Quick Fuel Entry and fuel-entry editing now use the same form and logic.
- Edit opens the Quick Fuel Entry form populated with the selected fuel entry.
- Save Entry changes to Save Edit during editing.
- Cancel Edit returns the form to normal Quick Fuel Entry mode.
- Saving an edit displays an acknowledgment, clears the form, exits edit mode, and remains on the Quick Fuel Entry page.
- Saving a new fuel entry displays an acknowledgment and returns to the Home Screen.
- Future Quick Fuel Entry modifications must also apply to the edit workflow.
- Date, Time, Miles, MPG, fuel, station, cost, receipt, and notes fields are shared by new-entry and edit workflows.
